            ...
            if ((empty($mail_auth) || !property_exists($mail_auth, 'access_token')) && !Options_Service::get_global_boolean_var('mail_staging_mode')) {
                ...
                if (is_wp_error($mail_access_token_app_only)) {
                    Log_Service::write_log('ERROR', sprintf(
                        ...
                        __METHOD__,
                        $phpmailer->Subject,
                        print_r($phpmailer->getToAddresses(), true)
                    ));
                    return;
                }
            }
        ...
        public static function mg_fetch($query, $body, $access_token, $method = 'POST', $headers = null)
        {
            ...
            $url = WordPress_Helpers::stripos($query, 'https://') === 0
                ...
                : sprintf(
                    ...
                    !empty($tld = Options_Service::get_global_string_var('tld')) ? $tld : '.com',
                    $graph_version,
        ...
        private static function validate_email_addresses($email_addresses, $level = 'DEBUG')
        {
            ...
            elseif (is_array($email_addresses)) {
                ...
                foreach ($email_addresses as $_email_address) {
                    ...
                    Log_Service::write_log($level, sprintf('%s -> Email address format invalid (%s)', __METHOD__, var_export($_email_address, true)));
                }
            }
             ...
             */
                ...
                Log_Service::write_log($level, sprintf('%s -> Email address format not recognized (%s)', __METHOD__, var_export($email_addresses, true)));
            }
            ...
            foreach ($_email_addresses as $_email_address) {
                ...
                try {
                    ...
                    Log_Service::write_log($level, sprintf('%s -> Invalid email address found (%s)', __METHOD__, var_export($_email_address, true)));
                    continue;
                }
            }
