    ...
    class Mail_Authorization_Helpers
    {
        ...
        public static function get_mail_auth_configuration($delete_delegated, $delete_app_only)
        {
            ...
            if (!empty($mail_auth) && property_exists($mail_auth, 'access_token')) {
                ...
                $scope = array_map(function ($item) use ($tld) {
                    $item = strtolower($item);
                    $item = false === WordPress_Helpers::stripos($item, "https://graph.microsoft$tld/") ? ("https://graph.microsoft$tld/" . $item) : $item;
                    return $item;
                }, $scope);
                $scope = array_map('strtolower', $scope);
                $mail_config->delegated_authorized = in_array("https://graph.microsoft$tld/mail.send", $scope);
                $mail_config->delegated_shared_authorized = in_array("https://graph.microsoft$tld/mail.send.shared", $scope);
                $mail_config->delegated_readwrite_authorized = in_array("https://graph.microsoft$tld/mail.readwrite", $scope);
                $mail_config->delegated_readwrite_shared_authorized = in_array("https://graph.microsoft$tld/mail.readwrite.shared", $scope);
            }
