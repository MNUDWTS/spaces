    ...
    class Wp_Config_Service
    {
        ...
        public static function apply_overrides(&$options)
        {
            ...
            if (!empty($options['enable_user_sync'])) {
                $jobs = is_array($options['user_sync_jobs']) ? array_map(function ($job) {
                    return $job;
                }, $options['user_sync_jobs']) : array();
            }
        ...
        public static function ensure_aad_options(&$options)
        {
            ...
            if (empty($wpo_aad)) {
                $aad_option_keys = Wp_Config_Service::get_aad_option_keys();
                $wpo_aad = array();
        ...
        public static function ensure_mail_options($options)
        {
            ...
            if (!empty($wpo_idps)) {
                ...
                if (sizeof($filtered_idps) == 1) {
                    ...
                    foreach ($mail_option_keys as $key) {
                        $wpo_mail[$key] = isset($filtered_idps[0][$key]) ? $filtered_idps[0][$key] : Wp_Config_Service::get_string_option($key, $options);
                    }
            ...
            if (!empty($wpo_aad)) {
                ...
                foreach ($mail_option_keys as $key) {
                    $wpo_mail[$key] = isset($wpo_aad[$key]) ? $wpo_aad[$key] : Wp_Config_Service::get_string_option($key, $options);
                }
            ...
            $mail_option_keys = Wp_Config_Service::get_mail_option_keys();
            $wpo_mail = array();
        ...
        public static function get_parseable_options($aad_options_only)
        {
            if (isset($GLOBALS['WPO_CONFIG']['options'])) {
                ...
                if ($aad_options_only) {
                    $parseable_options = array_filter($GLOBALS['WPO_CONFIG']['options'], function ($value, $key) use ($aad_option_keys, $mail_option_keys) {
                        return in_array($key, $aad_option_keys['strings']) || in_array($key, $aad_option_keys['bools']) || in_array($key, $mail_option_keys);
                    ...
                    $parseable_options = array_filter($GLOBALS['WPO_CONFIG']['options'], function ($value, $key) use ($keys_to_remove, $aad_option_keys, $mail_option_keys) {
                        return !in_array($key, $keys_to_remove) && !in_array($key, $aad_option_keys['strings']) && !in_array($key, $aad_option_keys['bools']) && !in_array($key, $mail_option_keys);
                    }, ARRAY_FILTER_USE_BOTH);

                    if (is_array($parseable_options['user_sync_jobs'])) {

                        for ($i = 0; $i < count($parseable_options['user_sync_jobs']); $i++) {
