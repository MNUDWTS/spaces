...
<div class='tw-relative'>
    <div class="tw-w-full tw-flex tw-justify-between tw-items-center tw-mb-4">
        <h2 class="tw-text-2xl tw-font-bold "><?= __('My Resources', 'spaces_mnu_plugin'); ?></h2>
        <a href="#" id="resource-create-button" class="tw-bg-transparent tw-border-none tw-flex tw-justify-center tw-items-center tw-gap-2">
            <h4 class="tw-hidden md:tw-block"><?= __('Add New Resource', 'spaces_mnu_plugin'); ?></h4>
    ...
    $args = array(
        ...
        'meta_query' => array(
            array(
                'key' => '_resource_responsible',
                'value' => sprintf('i:%d;', $current_user_id),
                'compare' => 'LIKE'
            ),
        ),
    );
    $user_resources = new WP_Query($args);
    if ($user_resources->have_posts()) : ?>
            ...
            <table class='tw-w-full tw-text-left tw-table-auto tw-min-w-max '>
                <thead>
                    <tr>
                        ...
                        <th class="tw-p-4 tw-border-b tw-border-slate-200 tw-bg-slate-50 tw-text-center">
                            <p class="tw-text-sm tw-font-normal tw-leading-none tw-text-slate-500"><?= __('Floor', 'spaces_mnu_plugin'); ?></p>
                        </th>
                        <th class="tw-p-4 tw-border-b tw-border-slate-200 tw-bg-slate-50 tw-text-center">
                ...
                <tbody>
                    <?php while ($user_resources->have_posts()) : $user_resources->the_post(); ?>
                        <?php
                        $post_id = get_the_ID();
                        $resource_type =  __('Not specified', 'spaces_mnu_plugin');
                        ...
                        <tr class="hover:tw-bg-slate-50 tw-border-b tw-border-slate-200">
                            ...
                            <td class="tw-p-4 tw-py-5 tw-text-center">
                                <p class="tw-block tw-font-semibold tw-text-sm tw-text-slate-800">
                                    <?= !empty($resource_floor) ? esc_html($resource_floor) : __('Not specified', 'spaces_mnu_plugin'); ?></p>
                            </td>
                            <td class="tw-p-4 tw-py-5 tw-text-center">
                                <p class="tw-block tw-font-semibold tw-text-sm tw-text-slate-800">
                                    <?= $resource_availability_enabled === 'true' ? __('Available', 'spaces_mnu_plugin') : __('Not Available', 'spaces_mnu_plugin'); ?></p>
                            </td>
                            <td class="tw-relative">
                                <div class="tw-flex tw-justify-end tw-items-center tw-w-full tw-h-full">
                                    <svg class="action-button" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div class="action-menu tw-absolute tw-space-y-2 tw-right-6 tw-top-0 tw-z-10 tw-mt-2 tw-w-fit tw-p-2 tw-origin-top-right tw-rounded-md tw-bg-white tw-shadow-lg tw-ring-1 tw-ring-black tw-ring-opacity-5 focus:tw-outline-none tw-hidden" role="menu" aria-orientation="vertical" aria-labelledby="action-button" tabindex="-1">
                                    <button class="edit-resource-button tw-w-full tw-bg-yellow-500 tw-text-white tw-px-2 tw-py-1 tw-rounded" data-resource-id="<?php echo get_the_ID(); ?>">
                                        <?= __('Edit', 'spaces_mnu_plugin'); ?>
                                    </button>
                                    <?php $nonce = wp_create_nonce('resource_nonce'); ?>
                                    <button data-resource-id="<?php echo get_the_ID(); ?>" data-nonce="<?php echo esc_attr($nonce); ?>" class="delete-resource-button tw-w-full tw-bg-red-500 tw-text-white tw-px-1 tw-py-1 tw-rounded">
                    ...
                    <?php endwhile; ?>
                    <script>
                        document.querySelectorAll('.action-button').forEach(function(actionButton) {
                            const actionMenu = actionButton.closest('td').querySelector('.action-menu');
        ...
        <div class="tw-flex tw-justify-end tw-items-center tw-px-4 tw-py-3 tw-overflow-x-auto">
            <?php
            $total_pages = $user_resources->max_num_pages;
    ...
    <?php else : ?>
        <p><?= __('No resources found.', 'spaces_mnu_plugin'); ?></p>
    <?php endif; ?>
...
<div id="confirmationModal" class="tw-fixed tw-inset-0 tw-z-50 tw-hidden tw-flex tw-items-center tw-justify-center tw-bg-black tw-bg-opacity-50">
    <div class="tw-bg-white tw-rounded-lg tw-overflow-hidden tw-shadow-lg tw-w-11/12 tw-max-w-lg">
        <div class="tw-p-6 tw-border-b tw-border-gray-200">
            <h3 class="tw-text-lg tw-font-medium tw-text-gray-900" id="modalTitle"><?= __('Confirm', 'spaces_mnu_plugin'); ?></h3>
            <p class="tw-text-sm tw-text-gray-600 tw-mt-2" id="modalMessage"></p>
        </div>
        <div class="tw-flex tw-justify-end tw-space-x-3 tw-p-4">
            <button id="cancelButton" class="tw-bg-gray-200 tw-text-gray-700 tw-px-4 tw-py-2 tw-rounded-lg tw-font-semibold" onclick="hideModal()"><?= __('Cancel', 'spaces_mnu_plugin'); ?></button>
            <button id="confirmButton" class="tw-bg-red-500 tw-text-white tw-px-4 tw-py-2 tw-rounded-lg tw-font-semibold"><?= __('Delete', 'spaces_mnu_plugin'); ?></button>
        </div>
    </div>
</div>
<script>
